...
const Gallery = () => {
  ...
  const handleUploadImages = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const imageFiles = files.filter((file: Blob) =>
    ...
    <section className="bg-white rounded-md border  shadow text-slate-700 w-full lg:w-[1000px]  mx-auto ">
      ...
      <div
        className={`p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 ${
          images.length > 0 ? 'child-width' : null
        }`}
      >
        {images.map((image, index) =>
          renderCard(image, index, getSelected, selected),
        )}
        {/* uploader  */}
        <div
          className={`${
            images.length === 0
              ? 'row-span-2 col-span-2 md:col-span-3 lg:col-span-5 flex justify-center items-center my-4'
